...
void FConnectionDrawingPolicy_Dialogue::Internal_DrawLineWithArrow(const FVector2D& StartAnchorPoint, const FVector2D& EndAnchorPoint, const FConnectionParams& Params)
{
...
void FConnectionDrawingPolicy_Dialogue::DetermineLinkGeometry(FArrangedChildren& ArrangedNodes, TSharedRef<SWidget>& OutputPinWidget,
	UEdGraphPin* OutputPin, UEdGraphPin* InputPin, FArrangedWidget*& StartWidgetGeometry, FArrangedWidget*& EndWidgetGeometry)
{
	if (UEdGraphNode_DialogueEdge* EdgeNode = Cast<UEdGraphNode_DialogueEdge>(InputPin->GetOwningNode()))
	{
